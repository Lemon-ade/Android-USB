...
public class MessageManager extends Service {
			 ...
			 public void run() {
				 ...
				 if(isCheckServerOn) {
					 if(clientIP==null) {
						 ...
						 LController lcontroller = LController.getInstance();
						 lcontroller.toast("connected : " + clientIP);
						 lcontroller.addClientIP(clientIP);
					 }
				 }
				 else {
	...
	public boolean sendToAndroidapp(String message) {
		...
		// use IME
			...
			// CONTROL - up/down/left/right / enter / esc
		...
		else { // IME setting view
			Log.d("MessagePCViewer", "IME not connect");
			LController.getInstance().toast("please set LController Keyboard!");
			openSetting();
			return false;
		}
	}
