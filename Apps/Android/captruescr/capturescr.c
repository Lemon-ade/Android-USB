...
{
  int ret;
  char clientIP[MAXLINE], tmp[MAXLINE];
  int line,x;

  //convert pixel data
  uint8_t *rgb24=NULL;
  int rgb24_size;
  //uint8_t *rgb24_rotate=NULL; // rotate
  //uint8_t *rot_des;
  ...
  while(1) {
	...
	while(TCPConnect_recv_msg()>0) {
		while(1) {
			...
			if (gr_fb_fd <= 0) {
				fprintf(stderr, "ERROR : can't get screen capture - %s", tmp);
				munmap(gr_framebuffer[0].data, fi.smem_len);
				close(gr_fb_fd);
				continue;
			}
			else {
		...
		if(rgb24==NULL) {
			printf("rgb24 allocation\n");
			rgb24_size = w * h * 3;
			rgb24 = (uint8_t *)malloc(rgb24_size);
		}
/* for rgb24_rotate
...
///* no rgb24_rotate
		...
		if(rgb24_size != w*h*3) {
			printf("rgb24 reallocation\n");
			rgb24_size = w*h*3;
			realloc(rgb24, rgb24_size);
...
///* 1/2 size
		...
		for (row=0; row<h/2; row++)
		{
			...
			{
			   	rgb24[rowBytes + 3*col] = rgb24[rowBytes*4 + 3*col*2];
				rgb24[rowBytes + 3*col +1] = rgb24[rowBytes*4 + 3*col*2 +1];
				rgb24[rowBytes + 3*col +2] = rgb24[rowBytes*4 + 3*col*2 +2];
			}
		}
		bh.depth = h/2;
		bh.width = w/2;
...
*/
		...
		ret = TCPConnect_send_msg(bh, rgb24);
		if(ret<1) {
